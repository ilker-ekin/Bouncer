...
export async function forward(req: Request, res: Response) {
  ...
  try {
    ...
    // Time the backend call (by tier) for latency percentiles. endTimer in a
    // finally so timeouts/errors are recorded too.
    const endTimer = startBackendTimer(req.tenant?.tier ?? "none");
    try {
